...
<div class="flex items-center justify-center my-5">
    ...
    <h1 class="font-bold text-xl md:text-3xl text-red-500">Detalles del Equipo</h1>
</div>

<div class="flex justify-center">
    <div class="w-full md:w-2/4 bg-white p-5 rounded-lg shadow-md">
        ...
        <div class="form-group mb-4">
            <p><strong class="block text-gray-700 font-bold mb-2">Fundación:</strong> {{ \Carbon\Carbon::parse($equipo->fundacion)->format('Y-m-d') }}</p>
        </div>
        <div class="form-group mb-4">
        ...
        <div class="flex justify-around mt-4">
            ...
            <form action="{{ route('equipos.destroy', $equipo->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este equipo?');">
                @csrf
                @method('DELETE')
...
<div class="flex justify-center my-5">
    <a href="{{ route('equipos.index') }}" class="text-white bg-red-500 hover:bg-red-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm md:text-base px-4 py-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Volver</a>
</div>
@endsection
